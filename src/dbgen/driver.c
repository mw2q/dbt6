...
long sd_date (int child, DSS_HUGE skip_count);
...
tdef tdefs[] =
{
	{"customer.tbl", "customers table", 30000, pr_cust, sd_cust, NONE, 0},
	{"date.tbl", "date table", 2556, pr_date, 0, NONE, 0},
	{"lineorder.tbl", "lineorder table", 150000, pr_order, sd_order, NONE, 0},
	{"part.tbl", "part table", 200000, pr_part, sd_part, NONE, 0},
	{"supplier.tbl", "suppliers table", 2000, pr_supp, sd_supp, NONE, 0},
};
...
usage (void)
{
	...
	fprintf (stderr, "-T d   -- generate dates ONLY\n");
	...
	switch (option)
	{
		...
		case 'T':				/* generate a specifc table */
			...
			case 'c':			/* generate customer ONLY */
				...
				bTableSet = 1;
				break;
			case 'd':			/* generate date table ONLY */
				table = 1 << DATE;
				bTableSet = 1;
				break;
